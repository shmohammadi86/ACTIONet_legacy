...
void *cluster::operator new(size_t size) {
  cluster *p =
      headOfFreeList; // p is now a pointer to the head of the free list
  ...
  if (p)
    ...
    // cmerk April 99 : Fibonacci like scheme to increase BLOCK_SIZE each time a
    // new block is demanded
