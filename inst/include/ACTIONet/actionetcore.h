...
namespace ACTIONetcore {
	...
	struct multilevel_archetypal_decomposition {
		vec landmark_cells; // Closest cell to each archetype. It's -1, if archetype is close to more than one vertex (i.e., it's hub or nonspecific)
		...
		mat archetype_profile; // gene x # archetype (after potentially removing hub archetypes). Computed as S*C_stacked
