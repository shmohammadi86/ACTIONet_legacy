 ...
 *  Read:: Data Structures and Algorithms for Nearest Neighbor Search in General Metric Spaces (Peter Yianilos)
 */
...
// Square-root of JS div is a metric: A new metric for probability distributions, IEEE Trans. Inform. Theory, 49(7), 1858-1860, 2003
double JSDiv_sqrt_distance(const DataPoint &t1, const DataPoint &t2) {
	// JS(p, q) = (KL(p || M) + KL(q || M)) / 2, where KL(p || M) = sum_d {P[i]*log(P[i] / M[i])}
	double mid = 0.0, p_div = 0.0, q_div = 0.0;	
    for (int d = 0; d < t1.dimensionality(); d++) {
        mid = (t1.x(d) + t2.x(d)) / 2.0;
        
        if(mid != 0 && t1.x(d) != 0) {
			p_div += t1.x(d)*std::log2(t1.x(d) / mid);
        }
        
        if(mid != 0 && t2.x(d) != 0) {
			q_div += t2.x(d)*std::log2(t2.x(d) / mid);        
		}		
    }
    
    double JSDiv = (p_div + q_div) / 2.0;
