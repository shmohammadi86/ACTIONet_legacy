...
# 			}))
    ...
    if ( (suppress.legend == FALSE) & !is.null(Annot) ) {
		...
		legend(legend.pos, legend = Annot, fill = Pal, cex = 0.5, bty = "n", plot = T)
    }
}
...
plot.ACTIONet.interactive <- function(ACTIONet.out, labels = NULL, transparency.attr = NULL, trans.z.threshold = -1, trans.fact = 2,
	node.size = 1, CPal = Pals.Snap88, enrichment.table = NULL, top.features = 7, Alt_Text = NULL, blacklist.pattern = "\\.|^RPL|^RPS|^MRP|^MT-|^MT|^RP|MALAT1|B2M|GAPDH", threeD = FALSE, title = "ACTIONet") {
    require(plotly)
    require(ACTIONet)
...
plot.ACTIONet.gradient <- function(ACTIONet.out, x, transparency.attr = NULL, trans.z.threshold = -0.5, trans.fact = 3, node.size = 1, CPal = "magma", prune = FALSE, title = "", alpha_val = 0.85, nonparameteric = FALSE, highlight = NULL) {
