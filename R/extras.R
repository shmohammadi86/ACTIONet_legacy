...
add.archetype.labels <- function(ACTIONet.out, k_min = 2, k_max = 20) {
	message("Adding archetype labels")
...
get.annotation.differential.scores <- function(ACTIONet.out, annotation.name, top.genes = 20, core = T) {
	...
	if(length(idx) == 0) {
		# R.utils::printf('Annotation %s not found\n', annotation.name)
    out = sprintf('Annotation %s not found\n', annotation.name)
    stop(out)
	}

	# R.utils::printf('Annotation found: name = %s, tag = %s\n', names(ACTIONet.out$annotations)[[idx]], ACTIONet.out$annotations[[idx]]$annotation.name)
  msg = sprintf('Annotation found: name = %s, tag = %s\n', names(ACTIONet.out$annotations)[[idx]], ACTIONet.out$annotations[[idx]]$annotation.name)
  message(msg)

	if(is.null(ACTIONet.out$annotations[[idx]]$DE.profile)) {
		stop("Please run compute.annotations.feature.specificity() first")
	}
	DE.profile = as.matrix(SummarizedExperiment::assays(ACTIONet.out$annotations[[idx]]$DE.profile)[["significance"]])
